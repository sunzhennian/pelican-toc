...
def generate_toc(content):
    if isinstance(content, contents.Static):
        return
    if os.path.splitext(str(content))[1] == '.md':
        _toc_run = content.metadata.get(
                'toc_run',
                content.settings['TOC']['TOC_RUN'])
        if not _toc_run == 'true':
            return
    if os.path.splitext(str(content))[1]=='.org':
        if re.findall("toc:(.*)",content.metadata['options'])[0].split(' ')[0]=='nil':
            return

    all_ids = set()
    tree = node = HtmlTreeNode(None, 'Table of contents', 'h0', '')
